<h1 align="center">
  Maria Laura Morales Villagra
</h1>

<h3 align="center">
  Frontend Developer • UI Engineer • UX-Oriented Development
</h3>

<p align="center">
  I build modern, responsive and user-centered web applications by combining software development with UI/UX design.
</p>

<p align="center">

<a href="https://www.linkedin.com/in/maria-laura-morales-6736a12ab">
  <img src="https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white">
</a>

<a href="https://github.com/mariamoralesv">
  <img src="https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white">
</a>

<a href="">
  <img src="https://img.shields.io/badge/Portfolio-000000?style=for-the-badge&logo=vercel&logoColor=white">
</a>

<a href="">
  <img src="https://img.shields.io/badge/Email-EA4335?style=for-the-badge&logo=gmail&logoColor=white">
</a>

</p>

---

# About Me

Frontend Developer with professional experience building websites and web applications for real clients.

I specialize in creating modern, responsive and visually polished interfaces using **React, Next.js, Astro and TypeScript**.

My biggest strength is combining **software development** with **graphic design and UX/UI**, allowing me to build products that are both technically solid and visually engaging.

---

# What I Bring

✔ 2+ years developing production-ready websites

✔ 5+ commercial web projects delivered

✔ 10+ clients worked with

✔ Up to 20% website performance optimization

✔ Experience integrating REST APIs

✔ Responsive & Mobile First Development

✔ UI/UX Design

✔ Agile (Kanban • Sprints)

✔ Git & GitHub Workflow

✔ Deployment and Hosting

---

# Tech Stack

<h3>Frontend</h3>

<p>
  <img src="https://skillicons.dev/icons?i=react,nextjs,astro,ts,js,html,css,tailwind"/>
</p>

<h3>Backend</h3>

<p>
  <img src="https://skillicons.dev/icons?i=python,django,nodejs,php"/>
</p>

<h3>Database</h3>

<p>
  <img src="https://skillicons.dev/icons?i=mysql,postgresql,sqlite"/>
</p>

<h3>Tools</h3>

<p>
  <img src="https://skillicons.dev/icons?i=git,github,vscode,figma,ai,ps"/>
</p>

---

# Featured Projects

## Pure

Complete Frontend Development

React • Next.js • Tailwind

✔ UI/UX

✔ Responsive Design

✔ SEO

✔ Performance

---

## Fymman Creativo

Agency Website

React • Astro

✔ Complete Frontend

✔ Branding

✔ Responsive

✔ Performance

---

## E-Primo Shop

E-commerce Platform

✔ Frontend Development

✔ REST APIs

✔ Webhooks

✔ Automation

---

# Professional Highlights

• Built complete websites from design to deployment

• Worked directly with business owners and stakeholders

• Created modern UI focused on usability

• Solved production issues and security vulnerabilities

• Combined branding, UI design and frontend development

---

# Currently Learning

TypeScript Best Practices

Frontend Architecture

Advanced React Patterns

Accessibility (WCAG)

Performance Optimization

---

# GitHub Stats

<p align="center">
  <img height="170" src="https://github-readme-stats.vercel.app/api?username=TUUSUARIO&show_icons=true&hide_border=true"/>

  <img height="170" src="https://github-readme-streak-stats.herokuapp.com/?user=TUUSUARIO&hide_border=true"/>
</p>

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=TUUSUARIO&layout=compact&hide_border=true"/>
</p>

---

<h2 align="center">
  Open to Frontend Developer opportunities
</h2>

<p align="center">
  I enjoy building interfaces that are clean, performant and delightful to use.
  Let's build something amazing together.
</p>
